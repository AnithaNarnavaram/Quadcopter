...
class Task():
    ...
    def get_reward(self):
        ...
        dx = abs(self.sim.pose[0] - self.target_pos[0])
        dy = abs(self.sim.pose[1] - self.target_pos[1])
        dz = abs(self.sim.pose[2] - self.target_pos[2])
        # create penalty, starting with 0.03
        penalty = 0.3*(np.sqrt((dx**2) + (dy**2) + (dz**2)))
        # add bonus
        bonus = 10.
        # calculate reward
